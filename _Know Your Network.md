...
Task:
 1. CoreTAAS must have the following SVIs
   - VLAN 1
       - IP address: 10.52.1.2
       - Description: DEFAULT-VLAN
       - Status: UP

   - VLAN 10
     - IP address: 10.52.10.2
     - Description: WIFI-VLAN
     - Status: UP

   - VLAN 50
     - IP address: 10.52.50.2
     - Description: CCTV-VLAN
     - Status: UP

   - VLAN 100
     - IP address: 10.52.100.2
...
!@CoreBABA
conf t
 ip dhcp excluded-address 10.52.1.1 10.52.1.100
 ip dhcp excluded-address 10.52.10.1 10.52.10.100
 ip dhcp excluded-address 10.52.50.1 10.52.50.100
...
!@CoreBABA
conf t
 ip dhcp excluded-address 10.52.100.1 10.52.100.100
...
conf t
 ...
 ip route 10.52.0.0 255.255.0.0 10.52.52.4 254
 end
~~~
...
conf t
 ip routing
 ip route 0.0.0.0 0.0.0.0 10.52.100.4 254
 end
~~~
...
!@CoreBABA
conf t
 ip route 0.0.0.0 0.0.0.0 10.52.52.1 254
 end
~~~
...
~~~
!@cmd
route add 10.0.0.0 mask 255.0.0.0 10.52.1.4
route add 200.0.0.0 mask 255.255.255.0 10.52.1.4
~~~
...
network 10.52.52.0 0.0.0.255 area 0
...
conf t
router ospf 1
router-id 10.52.52.4
network 10.52.0.0 0.0.255.255 area 0
int gi0/1
ip ospf network point-to-point
~~~
...
conf t
router ospf 1
router-id 10.52.100.8
network 10.52.100.0 0.0.0.255 area 0
end
~~~
...
~~~
!@cmd
ping 10.52.1.2                 CoreTAAS
ping 10.52.1.4                 CoreBABA
ping 10.52.100.8               CUCM
ping 10.52.52.1            EDGE
~~~
